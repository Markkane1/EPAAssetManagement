...
export async function listDocuments(ctx: RequestContext, filters: Record<string, unknown>) {
  ...
  return DocumentModel.find(query).sort({ created_at: -1 });
}
...
export async function getDocumentById(ctx: RequestContext, id: string) {
  ...
  if (!ctx.isHeadoffice && document.office_id.toString() !== ctx.locationId) {
    throw createHttpError(403, 'Access restricted to assigned office');
  }
  return document;
}
...
export async function uploadDocumentVersion(
  ...
  const lastVersion = await DocumentVersionModel.findOne({ document_id: documentId })
    .sort({ version_no: -1 })
    .exec();
  const nextVersion = lastVersion ? lastVersion.version_no + 1 : 1;
